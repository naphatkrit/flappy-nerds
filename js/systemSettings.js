...
SystemSettings.mySystem = {
    ...
    initializerSettings : {
        sphere:   new THREE.Vector4 ( 0.0, 0.0, 0.0, 5.0),
        color:    new THREE.Vector4 ( 1.0, 1.0, 1.0, 1.0 ),
        velocity: new THREE.Vector3 ( 0.0, 80.0, 0.0),
        lifetime: 7,
        size:     6.0,
    },
    ...
    updaterSettings : {
        externalForces : {
            gravity :     new THREE.Vector3( 0, 0, 0),
            attractors : [
                new THREE.Sphere( new THREE.Vector3(30.0, 30, 0), 30.0 ),
                new THREE.Sphere( new THREE.Vector3(-30.0, 30, 0), 30.0 )
            ],
        },
        collidables: {},
    ...
    particlesFreq: 500,
    createScene : function () {
        var sphere_geo = new THREE.SphereGeometry( 3.0, 32, 32 );
        var phong      = new THREE.MeshPhongMaterial( {color: 0x444444, emissive:0x442222, side: THREE.DoubleSide } );
        var sphere = new THREE.Mesh( sphere_geo, phong )

        sphere.position.set (-30.0, 30, 0);
        Scene.addObject( sphere );
...
SystemSettings.mySystem2 = {
    ...
    animationLoadFunction : function( geometry ) {
        ...
        mesh = new THREE.Mesh( geometry, new THREE.MeshLambertMaterial( { color: 0x801010, morphTargets: true, transparent:true, opacity:0.3 } ) );
        mesh.scale.set( 0.25, 0.25, 0.25 );
        // mesh.position.set( 0.0, 30.0, 0.0 );
        Scene.addObject( mesh );
        ParticleEngine.addMesh( mesh );
